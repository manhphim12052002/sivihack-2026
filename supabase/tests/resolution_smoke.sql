...
insert into sources (id, entity_type, entity_id, type, origin, filename, url, status, platform) values
  ('notice:SMOKE-1:01', 'tender', 'oeffentlichevergabe|SMOKE-1', 'EFORMS', 'EFORM_API',    null,             'https://example.test/notice/SMOKE-1',       'AVAILABLE', 'oeffentlichevergabe.de'),
  ('doc:aaa',           'tender', 'oeffentlichevergabe|SMOKE-1', 'PDF',    'PORTAL_FETCH', 'Bedingungen.pdf', 'https://example.test/docs/Bedingungen.pdf', 'AVAILABLE', 'example.test'),
  ('doc:bbb',           'tender', 'oeffentlichevergabe|SMOKE-1', 'PDF',    'PORTAL_FETCH', 'Vertrag.pdf',     'https://example.test/docs/Vertrag.pdf',     'AVAILABLE', 'example.test');

-- One Lot.
insert into lots (source, notice_id, notice_version, lot_id, title, cpv_main, published)
...
insert into observations (scope_type, scope_key, kind, attribute, extractor, source_id,
...
do $$
declare r observations_resolved%rowtype;
begin
  -- Case A
  select * into strict r from observations_resolved
  ...
  -- lots_latest picks the newest version
  insert into lots (source, notice_id, notice_version, lot_id, title)
  values ('oeffentlichevergabe', 'SMOKE-1', '02', 'LOT-0001', 'Smoke lot, corrected');
  assert (select count(*) from lots_latest where notice_id = 'SMOKE-1') = 1, 'lots_latest: one row per lot';
  assert (select notice_version from lots_latest where notice_id = 'SMOKE-1') = '02', 'lots_latest: newest version';
  assert (select procedure_key from lots_latest where notice_id = 'SMOKE-1') = 'oeffentlichevergabe|SMOKE-1', 'lots_latest: procedure_key';
  raise notice 'lots_latest ok';
end
$$;

-- Job queue: one queued job, claimed once, second claim returns null.
insert into ingest_jobs (id, payload) values ('job-smoke-1', '{"url": "https://example.test/notice/SMOKE-1"}');
do $$
...
-- Immutability: UPDATE and DELETE on observations must raise.
do $$
begin
  begin
    update observations set value_num = 99 where attribute = 'performance_guarantee';
    raise exception 'observations UPDATE was not rejected';
  exception when others then
    if sqlerrm not like 'observations rows are immutable%' then raise; end if;
    raise notice 'immutability ok (update): %', sqlerrm;
  end;
  begin
    delete from observations where attribute = 'performance_guarantee';
    raise exception 'observations DELETE was not rejected';
  exception when others then
    if sqlerrm not like 'observations rows are immutable%' then raise; end if;
